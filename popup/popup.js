...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Load saved state
  const stored = await chrome.storage.local.get(['enabled', 'badgeIcon', 'badgeTag']);
  ...
  if (tab?.url) {
    currentUrl = tab.url;
    try { hostnameEl.textContent = new URL(tab.url).hostname; }
    catch { hostnameEl.textContent = '—'; }
    fetchIP(tab.url);
  }

  // Toggle enabled
  toggleInput.addEventListener('change', () => {
    chrome.runtime.sendMessage({ type: 'TOGGLE_ENABLED', enabled: toggleInput.checked });
  });
